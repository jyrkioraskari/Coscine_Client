...
export async function fetchAimsApplicationProfiles({ query, includeDefinition = false, signal }) {
...
export async function fetchAimsApplicationProfileDefinition({ profile, signal }) {
  ...
  const profileWithDefinition = matchedProfileWithDefinition ?? profiles.find(getProfileDefinition);
  const shapes = profileWithDefinition ? buildSingleProfileDefinition(profileWithDefinition) : '';
